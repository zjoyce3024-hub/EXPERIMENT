...
alpha = 0.00001
beta = 0.00001
K = 200                 # Inner loop steps for PGD (Algorithm 1)
N = 5                   # Conjugate Gradient steps (Algorithm 2)
mu = 0.1                # Augmented parameter
r_noise = 1e-2          # Radius for uniform noise injection
rho_pgd = 1e-2          # Gradient norm threshold to trigger noise
delta_G = 1e-2          # Threshold for sufficient function decrease
delta_K = 10            # Epoch interval to check for SOSP

# x_net
num_T = 2

print("\n" + "=" * 80)
print(f"STARTING NEW EXPERIMENT SET FOR PROBE")
print("=" * 80 + "\n")

base_file_name = "PROBE"
...
for run in range(num_run):
    ...
    # Warm start initialization for CG
    v_prev = [torch.zeros_like(p) for p in lora_params]
    ...
    for t in range(num_step):
        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_math=True, enable_mem_efficient=False):
            ...
            for k in range(K):
                ...
                grad_norm = np.sqrt(sum(torch.sum(gy ** 2).item() for gy in g_y))

                if grad_norm <= rho_pgd and (k - tau) > delta_K:
                    y_anchor_loss = g_loss.item()
                    tau = k
                    with torch.no_grad():
                        for p in lora_params:
                            noise = (torch.rand_like(p) * 2 - 1) * r_noise
                            p.add_(noise)

                if (k - tau) == delta_K and y_anchor_loss is not None:
                    if g_loss.item() - y_anchor_loss > -delta_G:
                        break
            ...
            def HVP(v_tuple):
                g_y_v_dot = sum(torch.sum(gy * v).to(device) for gy, v in zip(g_y, v_tuple))
                hvp = torch.autograd.grad(g_y_v_dot, lora_params, retain_graph=True)
                return tuple(h + mu * v for h, v in zip(hvp, v_tuple))

            v = v_prev
            r = tuple(b_i - Av_i for b_i, Av_i in zip(f_y, HVP(v)))
            p = r
            rsold = sum(torch.sum(r_i * r_i).item() for r_i in r)

            for cg_step in range(N):
                Ap = HVP(p)
                alpha_cg = rsold / (sum(torch.sum(p_i * Ap_i).item() for p_i, Ap_i in zip(p, Ap)) + 1e-8)

                v = tuple(v_i + alpha_cg * p_i for v_i, p_i in zip(v, p))
                r = tuple(r_i - alpha_cg * Ap_i for r_i, Ap_i in zip(r, Ap))

                rsnew = sum(torch.sum(r_i * r_i).item() for r_i in r)
                if rsnew < 1e-8:
                    break
                p = tuple(r_i + (rsnew / rsold) * p_i for r_i, p_i in zip(r, p))
                rsold = rsnew

            v_prev = tuple(v_i.detach() for v_i in v)

            g_y_v_dot_final = sum(torch.sum(gy * v_i).to(device) for gy, v_i in zip(g_y, v))
            hypergrad_x = torch.autograd.grad(g_y_v_dot_final, weighting_model.parameters())

            with torch.no_grad():
                for param, hg in zip(weighting_model.parameters(), hypergrad_x):
                    param.data.add_(alpha * hg)
        ...
        with torch.no_grad():
            ...
            current_ll_grad_norm = np.sqrt(sum(torch.sum(gy ** 2).item() for gy in g_y))
        ...
        del outputs_x, outputs_y, outputs_y_val, g_loss, f_loss, current_ll_grad_norm
        del g_y, f_y, hypergrad_x
    ...
    try:
        ...
        del v, r, p, Ap
        del g_y_v_dot_final, alpha_cg, rsold, rsnew
    except NameError:
        pass
