...
print("\n" + "=" * 80)
print(f"STARTING NEW EXPERIMENT SET FOR MEHA (Single-loop Hessian-free)")
print("=" * 80 + "\n")
...
for run in range(num_run):
    ...
    theta_params = [p.clone().detach() for p in lora_params]
    ...
    for t in range(num_step):
        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_math=True, enable_mem_efficient=False):
            ...
            g_loss_xy = g_loss_WMSE(logits_flat, labels_flat, outputs_x)

            g_x_y = torch.autograd.grad(g_loss_xy, x_params)

            with torch.no_grad():
                y_backup = [p.data.clone() for p in lora_params]
                for p, th in zip(lora_params, theta_params):
                    p.data.copy_(th.data)

            outputs_y_th, outputs_x_th = get_outputs(None, None, main_model, weighting_model,
                                                     batch_train_input, batch_train_mask, one_tensor.to(device),
                                                     train_mask_tensor.to(device))
            logits_flat_th = outputs_y_th.logits.view(-1, outputs_y_th.logits.size(-1)).to(device)
            g_loss_th = g_loss_WMSE(logits_flat_th, labels_flat, outputs_x_th)

            g_theta = torch.autograd.grad(g_loss_th, lora_params)

            with torch.no_grad():
                for i, (th, g_th, y_bak) in enumerate(zip(theta_params, g_theta, y_backup)):
                    step_val = g_th + (1.0 / gamma) * (th - y_bak)
                    th.sub_(eta * step_val)
                    lora_params[i].data.copy_(th.data)

            outputs_y_th2, outputs_x_th2 = get_outputs(None, None, main_model, weighting_model,
                                                       batch_train_input, batch_train_mask, one_tensor.to(device),
                                                       train_mask_tensor.to(device))
            logits_flat_th2 = outputs_y_th2.logits.view(-1, outputs_y_th2.logits.size(-1)).to(device)
            g_loss_th2 = g_loss_WMSE(logits_flat_th2, labels_flat, outputs_x_th2)

            g_x_theta = torch.autograd.grad(g_loss_th2, x_params)

            with torch.no_grad():
                for p_x, gx_y, gx_th in zip(x_params, g_x_y, g_x_theta):
                    p_x.sub_(alpha * (gx_y - gx_th))

            with torch.no_grad():
                for p, y_bak in zip(lora_params, y_backup):
                    p.data.copy_(y_bak.data)

            outputs_y_next, outputs_x_next = get_outputs(None, None, main_model, weighting_model,
                                                         batch_train_input, batch_train_mask, one_tensor.to(device),
                                                         train_mask_tensor.to(device))
            logits_flat_next = outputs_y_next.logits.view(-1, outputs_y_next.logits.size(-1)).to(device)
            g_loss_next = g_loss_WMSE(logits_flat_next, labels_flat, outputs_x_next)

            g_y_y_next = torch.autograd.grad(g_loss_next, lora_params)
            ...
            with torch.no_grad():
                for p_y, y_bak, fy, gy_y, th_next in zip(lora_params, y_backup, f_y, g_y_y_next, theta_params):
                    dy = (1.0 / c) * fy + gy_y - (1.0 / gamma) * (y_bak - th_next)
                    p_y.data.copy_(y_bak - beta * dy)
        ...
        with torch.no_grad():
            train_loss_t.append(g_loss_next.item())
            test_loss_t.append(f_loss.item())

            current_ll_grad_norm = np.sqrt(sum(torch.sum(gy ** 2).to(device).item() for gy in g_y_y_next))
            ...
            if (t + 1) % 10 == 0:
                print(
                    f"Finished {t + 1} of {num_step} steps | Train Loss: {g_loss_next.item():.4f} | Val Loss: {f_loss.item():.4f} | LL Grad Norm: {current_ll_grad_norm:.4f}")

        del outputs_y, outputs_x, logits_flat
        del outputs_y_th, outputs_x_th, logits_flat_th
        del outputs_y_th2, outputs_x_th2, logits_flat_th2
        del outputs_y_next, outputs_x_next, logits_flat_next
        del outputs_y_val, logits_val_flat
        del g_loss_xy, g_loss_th, g_loss_th2, g_loss_next, f_loss
        del g_x_y, g_theta, g_x_theta, g_y_y_next, f_y
