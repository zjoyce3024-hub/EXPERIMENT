...
print("\n" + "=" * 80)
print(f"STARTING NEW EXPERIMENT SET FOR SUN-DSBO-SE")
print("=" * 80 + "\n")
...
for run in range(num_run):
    ...
    for t in range(num_step):
        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_math=True, enable_mem_efficient=False):
            ...
            g_loss_y = g_loss_WMSE(logits_flat, labels_flat, outputs_x)

            g_x_y = torch.autograd.grad(g_loss_y, x_params, retain_graph=True)
            g_y_y = torch.autograd.grad(g_loss_y, lora_params)
            ...
            g_x_th = torch.autograd.grad(g_loss_th, x_params, retain_graph=True)
            g_y_th = torch.autograd.grad(g_loss_th, lora_params)
            with torch.no_grad():
            ...
            with torch.no_grad():
                for px, gx_y, gx_th in zip(x_params, g_x_y, g_x_th):
                    d_x = gx_y - gx_th
                    px.sub_(alpha * d_x)

                for py, th, gy_th, fy, gy_y, y_bak in zip(lora_params, theta_params, g_y_th, f_y, g_y_y, y_backup):
                    d_theta = gy_th + (1.0 / gamma) * (th - y_bak)
                    d_y = mu_k * fy + gy_y - (1.0 / gamma) * (y_bak - th)
                    th.sub_(lambda_theta * d_theta)
                    py.data.copy_(y_bak - beta * d_y)
        ...
        with torch.no_grad():
            train_loss_t.append(g_loss_y.item())
            test_loss_t.append(f_loss.item())

            current_ll_grad_norm = np.sqrt(sum(torch.sum(gy ** 2).to(device).item() for gy in g_y_y))
            ...
            if (t + 1) % 10 == 0:
                print(
                    f"Finished {t + 1} of {num_step} steps | Train Loss: {g_loss_y.item():.4f} | Val Loss: {f_loss.item():.4f} | LL Grad Norm: {current_ll_grad_norm:.4f}")

        del outputs_y, outputs_x, logits_flat, g_loss_y
        del outputs_y_th, outputs_x_th, logits_flat_th, g_loss_th
        del outputs_y_val, logits_val_flat, f_loss
        del g_x_y, g_y_y, g_x_th, g_y_th, f_y
        del y_backup
