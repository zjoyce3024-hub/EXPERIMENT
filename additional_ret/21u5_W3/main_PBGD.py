...
print("\n" + "=" * 80)
print(f"STARTING NEW EXPERIMENT SET FOR PBGD (Baseline)")
print("=" * 80 + "\n")
...
for run in range(num_run):
    ...
    for t in range(num_step):
        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_math=True, enable_mem_efficient=False):
            ...
            penalty = sum(torch.sum(gy ** 2).to(device) for gy in g_y)
            x_params = list(weighting_model.parameters())
            grad_p_y = torch.autograd.grad(penalty, lora_params, retain_graph=True)
            grad_p_x = torch.autograd.grad(penalty, x_params)

            with torch.no_grad():
                for param, gx in zip(weighting_model.parameters(), grad_p_x):
                    param.sub_(alpha * gamma * gx)

                for p, fy, py in zip(lora_params, f_y, grad_p_y):
                    g = fy + gamma * py
                    p.sub_(beta * g)
        ...
        with torch.no_grad():
            ...
            current_ll_grad_norm = torch.sqrt(penalty).item()
        ...
        del f_y, g_y, penalty, grad_p_y, grad_p_x
